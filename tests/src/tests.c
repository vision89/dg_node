...
int main(int argc, char* argv[]) {
  ...
  DG_Node* generic_node = new_dg_node();                    // Node to test
  DG_Node* generic_node_2 = new_dg_node();                  // Node to test
  ...
  assert(generic_node->child == NULL);                 // The nodes next should default to NULL
  assert(generic_node_2->data == NULL);               // The nodes data should default to NULL
  assert(generic_node_2->child == NULL);               // The nodes next should default to NULL

  generic_node->data = test_data;                     // Set the data
  generic_node_2->data = test_data_2;                 // Set the data
  generic_node->child = generic_node_2;                // Set next
  ...
  assert(generic_node->child != NULL);                 // Next should no longer be null
  assert(generic_node->child == generic_node_2);       // Next should be the second node
  assert((generic_node->child)->data == test_data_2);  // Data should be the second test data
  assert((generic_node->child)->data != test_data);    // Data should not be the first test data
  assert((generic_node->child)->child == NULL);         // Next should be null as it isn't set
  ...
  generic_node->child = generic_node_2;
  ...
  generic_node->child = generic_node_2;             // Reset generic nodes next
